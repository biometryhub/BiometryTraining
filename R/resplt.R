...
resplt <- function(mod.obj) {
  ...
  qqplot.data <- function(vec = aa$stdres) {
    ...
    ggplot2::ggplot(vecdf, ggplot2::aes(sample = resids)) +
      ggplot2::stat_qq(colour = "black", fill = "aquamarine3", size = 2, shape = 21) +
  ...
  c <- ggplot2::ggplot(data = aa, mapping = ggplot2::aes(x = fitted, y = stdres)) +
    ggplot2::geom_point(colour = "black", fill = "aquamarine3", size = 2, shape = 21) +
    ggplot2::theme_bw() +
